...
defineProps<{ disabled?: boolean, editing?: boolean, provider?: string }>()
...
<template>
  <div class="grid gap-4">
    ...
    <BaseFormItem v-if="provider === 'opencode'" label="OpenCode 产品" required>
      <BaseSegmented v-model="form.tier" label="OpenCode 产品" :options="[{ label: 'Zen', value: 'zen' }, { label: 'Go', value: 'go' }]" :disabled="disabled" />
    </BaseFormItem>
    <BaseFormItem v-else label="Base URL" required>
      <template #extra>
    ...
    <BaseFormItem v-if="provider !== 'opencode'" label="传输方式">
